 ...
 */
function panels(model, cfg, names) {
  return names.map((name) => {
    try {
      return { name, svg: renderCard(model, name, cfg.theme?.[name]) };
    } catch (err) {
...
const page = (names, selected, error, meta, rendered = []) => `<!doctype html>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  a.tab { padding: 5px 12px; border: 1px solid #d1d9e0; border-radius: 6px;
          text-decoration: none; color: #1f2328; background: #fff; }
  a.tab[aria-current="true"] { background: #1f2328; color: #fff; border-color: #1f2328; }
  .meta { margin-left: auto; color: #59636e; font-size: 12px; }
  main { padding: 20px; display: flex; flex-direction: column; gap: 20px; }
  figure { margin: 0; background: #fff; border: 1px solid #d1d9e0; border-radius: 10px;
...
<header>
  <b>card</b>
  <a class="tab" href="/" ${selected ? "" : 'aria-current="true"'}>all</a>
  ${names
    .map(
      (n) =>
        `<a class="tab" href="/?template=${n}" ${selected === n ? 'aria-current="true"' : ""}>${n}</a>`,
    )
    .join("\n  ")}
  <a class="tab" href="${selected ? `/?template=${selected}&fresh=1` : "/?fresh=1"}">re-measure</a>
...
${rendered
  .map(
    (p) =>
      `<figure><figcaption>${p.name} &middot; <a href="/card.svg?template=${p.name}">card.svg?template=${p.name}</a></figcaption>` +
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      if (url.pathname === "/card.svg") {
        ...
        const svg = renderCard(model, template, cfg.theme?.[template]);
        res.writeHead(200, {
      ...
      const rendered = panels(model, cfg, selected ? [selected] : names);
      res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" });
      res.end(page(names, selected, null, meta, rendered));
    } catch (err) {
