...
/**
 * Pack `items` into lines joined by `sep`, breaking between items only.
 *
 * `wrap` breaks on spaces, which leaves a separator dangling at the end of a
 * line when the list reads "aic · edc · httprove ·". Items are the unit here,
 * so the separator never ends a line.
 *
 * @returns {string[]} lines, each already joined with `sep`
 */
export function packItems(items, maxWidth, size, face = "sans", sep = " · ") {
  const lines = [];
  let line = [];

  for (const item of items) {
    if (widthOf(item, size, face) > maxWidth) {
      throw new Error(
        `"${item}" is ${Math.ceil(widthOf(item, size, face))}px at ${size}px ` +
          `and cannot fit a ${Math.floor(maxWidth)}px column`,
      );
    }
    const candidate = [...line, item];
    if (widthOf(candidate.join(sep), size, face) <= maxWidth) {
      line = candidate;
    } else {
